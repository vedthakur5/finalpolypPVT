...
#             loss_p1p2 = np.square(np.subtract(nloss_P1, nloss_P2)).mean()
    ...
    torch.save(model.state_dict(), save_path +str(epoch)+ 'PolypPVT8.pth')
    ...
    if (epoch + 1) % 1 == 0:
        for dataset in ['Val']:   #'CVC-300','GlaS', 'CVC-ClinicDB', 'Kvasir', 'CVC-ColonDB', 'ETIS-LaribPolypDB',          ####### updated ########
            ...
            print(dataset, 'mean dice score: ', dataset_dice)
        ...
        if meandiceA > best:
            ...
            torch.save(model.state_dict(), PATH + 'PolypPVT8.pth')
            torch.save(model.state_dict(), PATH + 'PolypPVT-best8.pth')
            print('##############################################################################best', best)
            logging.info('##############################################################################best:{}'.format(best))
       
    
    
def save_checkpoint(state, filename = "my_checkpoint8.pth.tar"):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--decay_rate', type=float,
                        default=1e-2, help='decay rate of learning rate')
    ...
    if opt.optimizer == 'AdamW':
        ...
        optimizer = torch.optim.SGD(params, opt.lr, weight_decay=1e-2, momentum=0.9)
    ...
    if load_model:
        pth = f'/content/drive/MyDrive/DC/my_checkpoint8.pth.tar'
